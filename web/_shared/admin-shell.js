...
window.HKAdmin = (function () {
  ...
  // 양도는 회원 즉시 완료 — 관리자 승인 메뉴는 더 이상 사용하지 않음
  const NAV = [
  ...
  async function mountPage({ active, auth = true, render, afterMount }) {
    ...
    root.innerHTML = `<div class="hk-ashell" style="display:flex;min-height:100vh;min-height:100dvh">${sidebar(active)}<main class="hk-amain" style="flex:1;padding:32px 40px;max-width:1120px;box-sizing:border-box;min-width:0">${render()}</main></div>`;
    const logout = async () => {
