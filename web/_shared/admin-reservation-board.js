window.HKReservationBoard = (function () {
  ...
  function mount(container, board, vacationDates, showSectionHead) {
    ...
    function renderConfirmApplicant(a, i, opts) {
      const { allNoHistory, decided, canConfirm, slot } = opts;
      ...
      return `<div class="hk-card hk-card--pad" style="border:${borderW}px solid ${borderColor};background:${bg}">
        <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap">
          ...
          <div style="min-width:96px;display:flex;justify-content:flex-end">
            ${decided && a.status === "CONFIRMED" ? decidedStatusHtml(a, canConfirm, slot) : decided ? decidedStatusHtml(a, canConfirm, slot) : canConfirm ? `<button type="button" class="hk-btn hk-btn--${isTop && !allNoHistory ? "primary" : "secondary"} hk-btn--sm" data-rid="${a.reservation_id}">확정</button>` : `<span style="font-size:13px;color:var(--text-muted);white-space:nowrap">마감 후 확정</span>`}
          </div>
        </div>
      </div>`;
    }
    ...
    async function openConfirmModal(slotId) {
      ...
      try {
        ...
        const opts = { allNoHistory, decided, canConfirm, slot };
