...
dev_sync_backend_env() {
  grep -E '^(DATABASE_URL|REDIS_URL|NEO4J_URI|NEO4J_USER|NEO4J_PASSWORD|SECRET_KEY|DEBUG|API_PORT|SMTP_|APP_BASE_URL|SSO_PROVIDER|ENTRA_TENANT_ID|ENTRA_CLIENT_ID|ENTRA_CLIENT_SECRET|ENTRA_REDIRECT_URI|SSO_ALLOWED_DOMAIN|SSO_SUCCESS_PATH|PUBLIC_DATA_PORTAL_SERVICE_KEY|TEAMS_)=' "$DEV_ENV_ROOT/.env" > "$DEV_ENV_ROOT/backend/.env.local.tmp" 2>/dev/null || true
